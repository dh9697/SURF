...
// 시험 결과 삭제
// 시험 결과 생성
export function apiPostExamResult(
  examId,
  memberId,
  questionId,
  submittedAnswer
  ...
  return axios.post(
    `http://localhost:8080/api/exam-results/save/${examId}/${memberId}/${questionId}`,
    submittedAnswer,
    {
      headers: { Authorization: `Bearer ${token}` },
      "Content-Type": "application/json",
...
// 특정 시험 이력 조회
// 특정 회원의 시험 이력 조회
export function apiGetMyExamHistory(memberId) {
  return axios.get(
    `http://localhost:8080/api/exam-histories/list/member/${memberId}`,
    {
      headers: { Authorization: `Bearer ${token}` },
    }
  );
...
export function apiPostContentByCourse(courseId, contentDto) {
  return axios.post(
    `http://localhost:8080/api/content/save/${courseId}`,
    contentDto,
    {
      headers: { Authorization: `Bearer ${token}` },
    }
